...
import {
  Department,
  IDepartment,
} from "src/app/components/department/model/department.model";
...
export class DepartmentService {
  ...
  create(department: IDepartment): Observable<EntityDepartmentType> {
    ...
    return this.http.post<IDepartment>(url, department, {
      observe: "response",
    });
  }
  ...
  removeDepartment(id: any) {
    let url = `${this.url}department/${id}`;
    return this.http.delete<Department>(url, { observe: "response" });
  }

  updateDepartmentStatus(id) {
    let url = `${this.url}department/status/${id}`;
