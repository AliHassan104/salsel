...
export class ProductFieldService {
  ...
  constructor(private http: HttpClient) { }
    ...
    addProductFields(productFieldDto: IProductFieldDto): Observable<EntityProductFieldType> {
    let url = `${this.url}product-field`;
    return this.http
      .post<ProductFieldDto>(url, productFieldDto, { observe: 'response' })
    }

    getProductFields(): Observable<EntityProductFieldResponseType> {
      let url = `${this.url}product-field`;
      return this.http
        .get<ProductFieldDto[]>(url, { observe: 'response' })
    }
    ...
    removeProductFieldValue(id: any, pfvId?: any){
      ...
      return this.http
        .delete<ProductFieldDto>(url, { observe: 'response' })
    }
