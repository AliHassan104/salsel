...
public interface AwbRepository extends JpaRepository<Awb, Long> {
    ...
    @Query("SELECT a FROM Awb a WHERE a.assignedToUser.id = :userId And a.status = true ORDER BY a.id DESC")
    List<Awb> findAllInDesOrderByAssignedUserId(@Param("userId") Long userId);
    ...
    @Query("SELECT a FROM Awb a WHERE a.accountNumber = :accountNumber AND a.status = true")
    List<Awb> findByAccountNumber(@Param("accountNumber") String accountNumber);
    ...
    @Query("SELECT COUNT(a) FROM Awb a WHERE a.assignedToUser.name = :name")
